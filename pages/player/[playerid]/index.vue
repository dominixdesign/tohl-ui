<template>
  ...
  <div v-if="playerstats" key="player-career-display">
        ...
        <tfoot v-if="allSeasons.career" class="border-t-4 border-double border-secondary-500">
          ...
          <tr class="font-bold">
            ...
            >
              ...
              <span v-else>{{ get(allSeasons.career.all[typer], col, '&mdash;') }}</span>
            </td>
          </tr>
        </tfoot>
      </table>
...
watch(playerstats, (newPlayerstats) => {
  ...
  newPlayerstats.forEach((playerstat) => {
    ...
    if (!seasons.career[type][playerstat.team.teamid]) {
      seasons.career[type][playerstat.team.teamid] = Object.fromEntries(cols.map((col) => [col, 0]))
    }
